 ...
 */
public class TCSVParser extends TestCase {

	// coma-project\coma-engine\resources\Sources\CSV
	String directory = (new File ("")).getAbsolutePath() + "\\resources\\Sources\\CSV\\";
	
	static Object[][] data= {
			{"car.csv", 4, 3},
			{"dslr_table_1.csv", 73, 72},
			{"users.csv", 11, 10}
	};
	...
	public void singleImportWithoutDB(){
		CSVParser par = new CSVParser(false);
	...
	public void singleImport(){
		CSVParser par = new CSVParser(true);
		for (int i = 0; i < data.length; i++) {		
			String file = directory+(String) data[i][0];
			par.parseSingleSource(file);
		}
